    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Materi</h2>
            ...
            @if($materis->count() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('materi.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf @method('PATCH')
                    ...
                    <form action="{{ route('materi.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf @method('DELETE')
    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                ...
                <span class="text-gray-300">/</span>
                <a href="{{ route('materi.index') }}" class="hover:text-amber-600 transition">Materi</a>
                <span class="text-gray-300">/</span>
            ...
            <div class="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3">
                ...
                <p class="text-xs tracking-widest font-medium leading-relaxed text-red-700">
                    Data dalam arsip telah diarsipkan sementara. Gunakan tombol aksi untuk memulihkan atau menghapus data secara permanen beserta filenya.
                </p>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h3 class="font-semibold text-gray-800 text-sm tracking-wide">Data Materi Terhapus</h3>
                        <p class="text-gray-400 text-xs mt-0.5">
                            @if($isGuru)
                                Arsip materi milik Anda
                            @else
                                Arsip materi semua guru
                            @endif
                        </p>
                    </div>
                ...
                <div class="px-6 py-3 bg-gray-50 border-b border-gray-100">
                    <form method="GET" action="{{ route('materi.trash') }}" class="flex flex-wrap items-center gap-2">
                        {{-- Search --}}
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Materi</th>
                            ...
                            @forelse($materis as $materi)
                                <tr class="hover:bg-red-50/10 transition opacity-90">
                                    <td class="px-6 py-4 text-gray-400 text-xs font-mono">
                                        {{ str_pad($loop->iteration + ($materis->currentPage() - 1) * $materis->perPage(), 3, '0', STR_PAD_LEFT) }}
                                    </td>
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-400 mt-0.5">
                                            Pertemuan {{ $materi->Pertemuan->nomor_pertemuan ?? '-' }} • Tipe: {{ ucfirst($materi->tipe_materi) }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-medium text-gray-600 text-sm">
                                            {{ $materi->Pertemuan->JadwalBelajar->Kelas->nama_kelas ?? '-' }}
                                        </div>
                                        <div class="text-xs text-gray-400 mt-0.5">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 border border-red-100 rounded-lg">
                                            ...
                                            <span class="text-red-500 text-[11px] font-medium whitespace-nowrap">
                                                {{ $materi->deleted_at?->format('d M Y, H:i') ?? '-' }}
                                            </span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-1.5">
                                            {{-- Restore --}}
                                            <form action="{{ route('materi.restore', $materi->id) }}" method="POST"
                                            ...
                                            {{-- Force Delete --}}
                                            <form action="{{ route('materi.force-delete', $materi->id) }}" method="POST"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-300 text-xs">Tidak ada data materi yang dihapus sementara.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                @if($materis->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span class="font-semibold text-gray-700">{{ $materis->firstItem() }}–{{ $materis->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-gray-700">{{ $materis->total() }}</span>
                            entri
                        </p>
                        {{ $materis->links() }}
                    </div>
