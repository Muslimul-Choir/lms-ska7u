<x-app-layout>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach ($tugas as $t)
                                <tr class="hover:bg-amber-50/40 transition">
                                    ...
                                    <td class="px-6 py-4 text-gray-700">{{ $t->GuruMapel?->Guru?->user->name ?? '-' }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 border rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border-amber-200 uppercase">
                                            {{ $t->tipe_tugas }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-gray-700 text-sm">
                                        {{ \Carbon\Carbon::parse($t->batas_waktu)->format('d M Y H:i') }}</td>
                                    <td class="px-6 py-4">
    ...
    <!-- Modal Create -->
    <div id="modal-create" class="hidden fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title"
        role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"
                onclick="document.getElementById('modal-create').classList.add('hidden')"></div>
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="mt-4">
                                ...
                                <select name="id_pertemuan" id="id_pertemuan"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                                    onchange="updateMapelAndGuruMapel()">
                                    <option value="">-- Pilih Pertemuan --</option>
                                    @foreach ($pertemuans as $p)
                                        <option value="{{ $p->id }}"
                                            data-guru-mapel-id="{{ $p->JadwalBelajar?->id_guru_mapel }}"
                                            data-mapel-id="{{ $p->JadwalBelajar?->id_mapel }}">Pertemuan
                                            {{ $p->nomor_pertemuan }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mt-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div class="mt-4">
                                ...
                                <select name="id_guru_mapel" id="id_guru_mapel"
                                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    <option value="">-- Auto dari Pertemuan --</option>
                                    @foreach ($guruMapels as $gm)
                                        <option value="{{ $gm->id }}">{{ $gm->Mapel?->nama_mapel ?? '-' }} -
                                            {{ $gm->Guru?->user->name ?? '-' }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div id="link-input-section" class="mt-4 hidden">
                            ...
                            <input type="url" name="file_url_link" id="file_url_link" placeholder="https://..."
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                        </div>
