<x-app-layout>
    ...
    <style>
        @media print {
            ...
            nav, header, .breadcrumb, button, a[href*="tugas.index"], .aksi-cell {
    ...
    <div class="py-7 bg-gray-50 min-h-screen" x-data="penilaianModal()">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="mb-5 flex items-center justify-between gap-3">
                    <a href="{{ route('tugas.index') }}" class="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2.5 px-4 rounded-lg transition-all">
                    ...
                    <div class="flex items-center gap-2">
                        <!-- Print Button -->
                        <button onclick="window.print()" class="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2.5 px-4 rounded-lg transition-all shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        ...
                        <!-- Export PDF Button -->
                        <a href="{{ route('tugas.rekap.export-pdf', $tugas) }}" class="inline-flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2.5 px-4 rounded-lg transition-all shadow-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                ...
                <div class="mb-5 p-5 bg-amber-50 border border-amber-100 rounded-xl">
                    ...
                    <div class="grid grid-cols-3 gap-4 text-sm">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-xs font-bold uppercase">{{ ucfirst($tugas->tipe_tugas) }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="text-gray-500 font-semibold">Batas Waktu:</span>
                            <span class="text-gray-700">{{ \Carbon\Carbon::parse($tugas->batas_waktu)->format('d M Y, H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4 mb-5">
                    <div class="bg-blue-50 border border-blue-100 p-5 rounded-xl text-center">
                        ...
                        <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Total Siswa</div>
                    </div>
                    <div class="bg-emerald-50 border border-emerald-100 p-5 rounded-xl text-center">
                        <div class="text-3xl font-bold text-emerald-700">{{ $statistik['sudah_mengumpulkan'] }}</div>
                        <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Sudah Mengumpulkan</div>
                    </div>
                    <div class="bg-purple-50 border border-purple-100 p-5 rounded-xl text-center">
                        ...
                        <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Sudah Dinilai</div>
                    </div>
                </div>

                @if(!$isCreator)
                    <div class="p-4 mb-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2">
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">No</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Nama Siswa</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Status Pengumpulan</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">File Lampiran</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Status Penilaian</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Nilai</th>
                                @if($isCreator)
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-amber-50/40 transition" data-siswa-id="{{ $item['siswa']->id }}" data-submission-id="{{ $item['submission']?->id }}">
                                <td class="px-6 py-4 text-gray-400 text-xs font-mono">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</td>
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-gray-900 text-sm">{{ $item['siswa']->nama_lengkap }}</div>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 border rounded-full text-[10px] font-bold bg-red-50 text-red-700 border-red-200">
                                            Belum Mengumpulkan
                                        </span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    @if($item['submission'] && $item['submission']->file_url)
                                        @if(str_starts_with($item['submission']->file_url, 'http'))
                                            <a href="{{ $item['submission']->file_url }}" target="_blank" class="text-amber-600 hover:text-amber-700 font-semibold inline-flex items-center gap-1">
                                        ...
                                        @else
                                            <a href="{{ route('pengumpulan-tugas.download', $item['submission']) }}" class="text-amber-600 hover:text-amber-700 font-semibold inline-flex items-center gap-1">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                ...
                                <td class="px-6 py-4 status-cell">
                                    @if($item['penilaian'])
                                        <span class="inline-flex items-center px-2.5 py-1 border rounded-full text-[10px] font-bold bg-purple-50 text-purple-700 border-purple-200">
                                            Sudah Dinilai
                                        </span>
                                ...
                                <td class="px-6 py-4 nilai-cell">
                                    @if($item['penilaian'])
                                        <span class="font-bold text-purple-700 text-sm">{{ number_format($item['penilaian']->nilai, 1) }}</span>
                                    @else
                                        <span class="text-gray-300 text-xs">-</span>
                                    @endif
                                </td>
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input 
                                type="number" 
                                x-model="currentData.nilai" 
                                :max="currentData.nilai_maksimal"
                                min="0"
                                step="0.1"
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Catatan Guru (Opsional)</label>
                            <textarea 
                                x-model="currentData.catatan_guru" 
                                rows="4"
                                maxlength="1000"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div x-show="errorMessage" class="mb-4 p-3 text-sm text-red-700 bg-red-100 rounded-lg" x-text="errorMessage"></div>
                    </div>
            ...
            async submitPenilaian() {
                ...
                try {
                    ...
                    if (response.data.success) {
                        // Update DOM (Req 3.6)
                        const row = document.querySelector(`tr[data-submission-id="${this.currentData.id_pengumpulan_tugas}"]`);
                        if (row) {
                            // Update status
                            const statusCell = row.querySelector('.status-cell');
                            statusCell.innerHTML = '<span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-purple-100 text-purple-800">Sudah Dinilai</span>';

                            // Update nilai
                            const nilaiCell = row.querySelector('.nilai-cell');
                            nilaiCell.innerHTML = `<span class="font-bold text-purple-700">${parseFloat(response.data.nilai).toFixed(1)}</span>`;

                            // Update button to "Edit Nilai" (Req 3.7, 3.8)
                            const aksiCell = row.querySelector('.aksi-cell button');
                            if (aksiCell) {
                                aksiCell.innerHTML = '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg> Edit Nilai';
                                aksiCell.className = 'text-blue-600 hover:text-blue-900 font-semibold inline-flex items-center gap-1';
                            }
                        }
                        ...
                        successDiv.className = 'flex items-center justify-between px-4 py-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-sm mb-4';
                    ...
                    if (error.response) {
                        this.errorMessage = error.response.data.message || 'Terjadi kesalahan saat menyimpan penilaian.';
                    } else {
