<x-app-layout>
    ...
    <div class="bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto space-y-5">
            ...
            {{-- Search & Filter Bar --}}
            <form method="GET" action="{{ route('pengumpulan-tugas.index') }}"
                ...
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                        placeholder="Cari nama siswa, NIS, atau judul tugas..."
                ...
                <div class="flex flex-wrap sm:flex-nowrap items-center gap-3">
                    <select name="status"
                        ...
                        <option value="dinilai" {{ request('status') == 'dinilai' ? 'selected' : '' }}>Sudah Dinilai
                        </option>
                        <option value="belum_dinilai" {{ request('status') == 'belum_dinilai' ? 'selected' : '' }}>Belum
                            Dinilai</option>
                    </select>
                            ...
                            @forelse($pengumpulanTugas as $item)
                                <tr class="hover:bg-amber-50/40 transition">
                                    <td class="px-6 py-4 text-gray-400 text-xs font-mono">
                                        {{ str_pad(($pengumpulanTugas->currentPage() - 1) * $pengumpulanTugas->perPage() + $loop->iteration, 2, '0', STR_PAD_LEFT) }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-semibold text-gray-900 text-sm">
                                            {{ $item->siswa->nama_lengkap ?? 'Unknown' }}</div>
                                        <div class="text-xs text-gray-500">NIS: {{ $item->siswa->nis ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-semibold text-gray-800 text-sm">
                                            {{ $item->tugas->judul ?? '-' }}</div>
                                        <div class="text-xs text-gray-500">
                                            {{ $item->tugas->GuruMapel?->Mapel?->nama_mapel ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        @if ($item->file_url)
                                            @if (str_starts_with($item->file_url, 'http'))
                                                <a href="{{ $item->file_url }}" target="_blank"
                                                    class="inline-flex items-center gap-1.5 text-amber-600 hover:text-amber-700 font-semibold text-sm">
                                            ...
                                            @else
                                                <a href="{{ route('pengumpulan-tugas.download', $item) }}"
                                                    class="inline-flex items-center gap-1.5 text-amber-600 hover:text-amber-700 font-semibold text-sm">
                                                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24"
                                                        stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        @endif
                                        @if ($item->catatan)
                                            <div class="text-xs mt-1 text-gray-500 italic max-w-xs truncate"
                                                title="{{ $item->catatan }}">"{{ $item->catatan }}"</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        @if ($item->penilaian)
