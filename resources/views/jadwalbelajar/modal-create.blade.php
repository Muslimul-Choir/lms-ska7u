            ...
            <form action="{{ route('jadwalbelajar.store') }}" method="POST" class="px-6 py-5 space-y-4">
                ...
                <div>
                    ...
                    <select name="hari"
                            class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700 bg-white
                                   ...
                                   @error('hari') border-red-400 bg-red-50 @else border-slate-200 @enderror">
                ...
                <div>
                    ...
                    <select name="id_jam"
                            class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700 bg-white
                                   focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]/30 focus:border-[#1B3A6B] transition
                                   @error('id_jam') border-red-400 bg-red-50 @else border-slate-200 @enderror">
                        <option value="">-- Pilih Jam --</option>
                        @foreach($jamBelajars as $jam)
                            <option value="{{ $jam->id }}" {{ old('id_jam') == $jam->id ? 'selected' : '' }}>
                    ...
                    <select name="id_kelas"
                        ...
                        @foreach($kelas as $k)
                            <option value="{{ $k->id }}" {{ old('id_kelas') == $k->id ? 'selected' : '' }}>
                                {{ ($k->Tingkatan->nama_tingkatan ?? '') . ' ' . ($k->Jurusan->nama_jurusan ?? '') . ' ' . ($k->Bagian->nama_bagian ?? '') }}
                            </option>
                        @endforeach
                    </select>
                    @error('id_kelas')
...
<div>
    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">
        Guru Mata Pelajaran <span class="text-red-400">*</span>
    </label>
    <select name="id_guru_mapel"
            class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700 bg-white
                   focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]/30 focus:border-[#1B3A6B] transition
                   @error('id_guru_mapel') border-red-400 bg-red-50 @else border-slate-200 @enderror">
        <option value="">-- Pilih Guru --</option>
        @foreach($guruMapels as $guru)
            <option value="{{ $guru->id }}" {{ old('id_guru_mapel') == $guru->id ? 'selected' : '' }}>
