    ...
    <div class="bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto space-y-5">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                {{-- Card Header --}}
                <div
                    class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-[#6B1A1A] to-[#B91C1C]">
                    <div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($jadwals as $jadwal)
                                <tr class="hover:bg-red-50/40 transition group">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-md border text-xs font-semibold bg-slate-100 text-slate-400 border-slate-200 line-through decoration-red-300">
                                            {{ $jadwal->hari }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            {{-- Restore --}}
                                            <form action="{{ route('jadwalbelajar.restore', $jadwal->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                            ...
                                            {{-- Force Delete --}}
                                            <form action="{{ route('jadwalbelajar.force-delete', $jadwal->id) }}"
                                                method="POST"
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-slate-300 text-xs">Tidak ada data jadwal belajar yang
                                                dihapus sementara</p>
                                        </div>
                                    </td>
                                </tr>
                ...
                @if ($jadwals->hasPages())
                    <div
                        ...
                        <p class="text-xs text-slate-500">
                            Menampilkan
                            <span
                                class="font-semibold text-slate-700">{{ $jadwals->firstItem() }}–{{ $jadwals->lastItem() }}</span>
                            dari
