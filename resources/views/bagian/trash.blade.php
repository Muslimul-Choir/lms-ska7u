    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Bagian</h2>
            ...
            @if ($bagians->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('bagian.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('bagian.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
                                ...
                                <tr class="hover:bg-red-50/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            {{-- Force Delete --}}
                                            <form action="{{ route('bagian.force-delete', $bagian->id) }}"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            @else
                                                <p class="text-gray-300 text-xs">Tidak ada data bagian yang dihapus
                                                    sementara</p>
                                            @endif
                                        </div>
                                    </td>
                ...
                @if ($bagians->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span
                                class="font-semibold text-gray-700">{{ $bagians->firstItem() }}–{{ $bagians->lastItem() }}</span>
                            dari
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('bagian').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                showConfirmRestoreAll('Bagian').then((result) => {
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                showConfirmForceDeleteAll('Bagian').then((result) => {
