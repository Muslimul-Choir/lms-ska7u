<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Master Bagian</h2>
    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                ...
                <span class="text-gray-300">/</span>
                <span class="text-gray-600 font-semibold">Bagian</span>
            </nav>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Nama Bagian</th>
                        ...
                        <tbody id="bagianTableBody" class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-amber-50/40 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-semibold text-gray-800 text-sm">{{ $bagian->nama_bagian }}</span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">Belum ada data bagian</p>
                                            <p class="text-gray-300 text-xs">Klik <span class="font-semibold text-gray-400">+ Tambah Bagian</span> untuk mulai menambahkan</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                {{-- Pagination --}}
                @if ($bagians->hasPages())
                    <div id="paginationContainer" class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span class="font-semibold text-gray-700">{{ $bagians->firstItem() }}–{{ $bagians->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-gray-700">{{ $bagians->total() }}</span>
                            entri
                        </p>
                        {{ $bagians->links() }}
                    </div>
   ...
   <script>
    ...
    btnSearch.addEventListener('click', () => {
        window.location.href = `{{ route('bagian.index') }}?search=${encodeURIComponent(searchInput.value)}`;
    });

    if (btnReset) {
        btnReset.addEventListener('click', () => {
            window.location.href = `{{ route('bagian.index') }}`;
        });
    }
    ...
    document.getElementById('btnTambahBagian').addEventListener('click', () => {
        ...
        document.querySelectorAll('.btn-edit').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('formEdit').action =
                    `/bagian/${this.dataset.id}`;

                document.getElementById('editNamaBagian').value =
                    this.dataset.nama;
