...
<aside :class="sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'"
    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5 scrollbar-thin"
        ...
        @if(
            ...
            (Auth::user()->role === 'guru' && in_array(Auth::user()->guru?->status_pengajar, ['walikelas', 'keduanya']))
        )
            {{-- Siswa --}}
        ...
        <a href="{{ route('penilaian.index') }}"
            class="group flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 {{ navActive('penilaian.*') }} {{ navActive('pengumpulan-tugas.*') }}">
            ...
            @if (navIndicator('penilaian.*') || navIndicator('pengumpulan-tugas.*'))
                <span class="flex-shrink-0 w-1 h-4 rounded-full bg-yellow-400"></span>
            @endif
        </a>
