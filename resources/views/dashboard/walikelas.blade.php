...
<div class="space-y-5"
     x-data="{
        ...
        activePertemuanMap: {
            ...
            {{ $mp['id_mapel'] }}: {{ !empty($mp['pertemuan']) ? ($mp['pertemuan'][0]['id'] ?? 'null') : 'null' }},
            @endforeach
        },
    ...
    <nav class="flex items-center gap-1.5 text-xs text-slate-400 font-medium">
        <span class="text-amber-500 font-semibold">Dashboard</span>
        <span class="text-slate-300">/</span>
        <span class="text-slate-500">Monitoring Wali Kelas</span>
    </nav>
    ...
    @if($waliKelasData)
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div x-show="activeMapel === {{ $mp['id_mapel'] }}" x-cloak>
                ...
                <div class="px-5 py-3 border-b border-slate-100 space-y-3">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($mp['pertemuan'] as $pt)
                            <button
                                @click="setPertemuan({{ $mp['id_mapel'] }}, {{ $pt['id'] ?? 'null' }})"
                ...
                @foreach($mp['pertemuan'] as $pt)
                <div x-show="getActivePertemuan({{ $mp['id_mapel'] }}) === {{ $pt['id'] ?? 'null' }}" x-cloak class="p-5 space-y-6">
                    ...
                    @if(!$pt['ada_konten'])
                        ...
                        <div class="rounded-lg border border-dashed border-slate-200 py-10 text-center">
                            ...
                            <p class="text-sm font-medium text-slate-400">Tidak ada tugas/kuis di pertemuan ini</p>
                        </div>
                    ...
                    @else
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <h4 class="text-xs font-bold text-slate-600 uppercase tracking-wide">Rekap Tugas</h4>
                                <span class="text-[10px] px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-200 font-semibold">
                                ...
                                <table class="w-full text-xs border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50">
                                            {{-- Sticky: No --}}
                                            <th class="sticky left-0 z-10 bg-slate-50 text-left px-3 py-3 font-semibold text-slate-500 uppercase tracking-wide text-[10px] w-8 border-b border-r border-slate-200">
                                                No
                                            </th>
                                            {{-- Sticky: Nama --}}
                                            <th class="sticky left-8 z-10 bg-slate-50 text-left px-4 py-3 font-semibold text-slate-500 uppercase tracking-wide text-[10px] min-w-[160px] border-b border-r border-slate-200">
                                                Nama Siswa
                                            </th>
                                            {{-- Kolom tugas (scrollable) --}}
                                            @foreach($pt['tugas'] as $tugas)
                                                <th class="text-center px-3 py-3 font-semibold text-slate-500 uppercase tracking-wide text-[10px] min-w-[130px] border-b border-slate-200">
                                                    <div class="truncate max-w-[120px] mx-auto" title="{{ $tugas->judul }}">
                                                        {{ Str::limit($tugas->judul, 16) }}
                                                    </div>
                                                    <div class="text-[9px] text-slate-400 font-normal normal-case tracking-normal mt-0.5">
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        @foreach($siswaList as $i => $siswa)
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                {{-- Sticky: No --}}
                                                <td class="sticky left-0 z-10 bg-white px-3 py-3 text-slate-400 font-medium border-r border-slate-100 hover:bg-slate-50">
                                                    {{ $i + 1 }}
                                                </td>
                                                {{-- Sticky: Nama --}}
                                                <td class="sticky left-8 z-10 bg-white px-4 py-3 font-semibold text-[#0F2145] whitespace-nowrap border-r border-slate-100 hover:bg-slate-50">
                                                    {{ $siswa->nama_lengkap }}
                                                </td>
                                                {{-- Nilai per tugas --}}
                                                @foreach($pt['tugas'] as $tugas)
                                                    @php
                                                        ...
                                                        $nilai       = $pengumpulan?->first()?->Penilaian?->nilai ?? null;
                                                    @endphp
                                                    <td class="px-3 py-3 text-center">
                                                        @if($pengumpulan)
                                                            @if($nilai !== null)
                                                                <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-[10px] font-bold
                                                                    {{ $nilai >= 75 ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-rose-50 text-rose-600 border border-rose-200' }}">
                                                                    {{ $nilai }}
                                                                </span>
                                                            @else
                                                                <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-blue-50 text-blue-500 border border-blue-200">
                                                                    Dikumpul
                                                                </span>
                                                            @endif
                                                        @else
                                                            <span class="inline-flex items-center justify-center px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 text-slate-400 border border-slate-200">
                                                                Belum
                                                            </span>
                                                        @endif
                                                    </td>
                            ...
                            <div class="flex flex-wrap gap-3 mt-2">
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-emerald-500"></span><span class="text-[10px] text-slate-400">Nilai ≥ 75</span></div>
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-rose-500"></span><span class="text-[10px] text-slate-400">Nilai &lt; 75</span></div>
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-blue-500"></span><span class="text-[10px] text-slate-400">Dikumpul, belum dinilai</span></div>
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-slate-300"></span><span class="text-[10px] text-slate-400">Belum dikumpul</span></div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div>
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <h4 class="text-xs font-bold text-slate-600 uppercase tracking-wide">Rekap Kuis</h4>
                                <span class="text-[10px] px-2 py-0.5 bg-violet-50 text-violet-600 rounded-full border border-violet-200 font-semibold">
                                    {{ $pt['kuis']->count() }} kuis
                                </span>
                            </div>
                            ...
                            <div class="overflow-x-auto rounded-lg border border-slate-200">
                                <table class="w-full text-xs border-collapse">
                                    <thead>
                                        <tr class="bg-slate-50">
                                            ...
                                            @foreach($pt['kuis'] as $kuis)
                                                <th class="text-center px-3 py-3 font-semibold text-slate-500 uppercase tracking-wide text-[10px] min-w-[130px] border-b border-slate-200">
                                                    <div class="truncate max-w-[120px] mx-auto" title="{{ $kuis->judul }}">
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        @foreach($siswaList as $i => $siswa)
                                            <tr class="hover:bg-slate-50 transition-colors">
                                                <td class="sticky left-0 z-10 bg-white px-3 py-3 text-slate-400 font-medium border-r border-slate-100">
                                                    {{ $i + 1 }}
                                                </td>
                                                <td class="sticky left-8 z-10 bg-white px-4 py-3 font-semibold text-[#0F2145] whitespace-nowrap border-r border-slate-100">
                                                    {{ $siswa->nama_lengkap }}
                                                </td>
                                                @foreach($pt['kuis'] as $kuis)
                                                    @php
                            ...
                            <div class="flex flex-wrap gap-3 mt-2">
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-emerald-500"></span><span class="text-[10px] text-slate-400">Nilai ≥ 75</span></div>
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-rose-500"></span><span class="text-[10px] text-slate-400">Nilai &lt; 75</span></div>
                                <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-slate-300"></span><span class="text-[10px] text-slate-400">Belum mengerjakan</span></div>
                            </div>
                        </div>
                        @endif
