    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Absensi</h2>
            ...
            @if ($absensis->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('absensi.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('absensi.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                ...
                <span class="text-gray-300">/</span>
                <a href="{{ route('absensi.index') }}" class="hover:text-amber-600 transition">Absensi</a>
                <span class="text-gray-300">/</span>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100">
                    ...
                    {{-- Filter & Search --}}
                    <form method="GET" action="{{ route('absensi.trash') }}" class="flex flex-wrap items-center gap-2">
                                ...
                                <tr class="hover:bg-red-50/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center gap-1.5 text-gray-400 text-sm">
                                            <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-red-100 text-red-400 text-[10px] font-bold">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @php
                                            $statusMap = [
                                                'hadir' => ['bg-emerald-50', 'text-emerald-400', 'border-emerald-100', 'Hadir'],
                                            ...
                                            [$bg, $text, $border, $label] = $statusMap[$absensi->status] ?? ['bg-gray-50','text-gray-300','border-gray-100',$absensi->status];
                                        @endphp
                                        <span class="inline-flex items-center px-2.5 py-1 {{ $bg }} {{ $text }} border {{ $border }} text-[10px] font-bold rounded-full opacity-75">
                                            {{ $label }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-gray-400 text-sm max-w-xs truncate">
                                        @if($absensi->keterangan)
                                            <span class="line-through decoration-red-300">
                                                {{ \Illuminate\Support\Str::limit($absensi->keterangan, 35) }}
                                            </span>
                                        @else
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-300 text-xs">Tidak ada data absensi yang dihapus sementara</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('absensi').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                showConfirmRestoreAll('Absensi').then((result) => {
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                showConfirmForceDeleteAll('Absensi').then((result) => {
