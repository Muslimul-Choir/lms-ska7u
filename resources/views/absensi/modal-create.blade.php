        ...
        <div style="background:#fff; border-radius:18px; box-shadow:0 24px 60px rgba(107,26,43,0.22), 0 4px 16px rgba(0,0,0,0.08); overflow:hidden; border:1px solid rgba(107,26,43,0.1);">
            ...
            <div style="padding:18px 24px; background:linear-gradient(135deg,#6B1A2B 0%,#4A0F1E 55%,#2D0810 100%); display:flex; align-items:center; justify-content:space-between; position:relative; overflow:hidden;">
                ...
                <div style="display:flex; align-items:center; gap:12px; position:relative;">
                    ...
                    <div>
                        <h3 style="color:#fff; font-weight:700; font-size:15px; margin:0 0 2px;">Tambah Absensi</h3>
                        <p style="color:rgba(255,255,255,0.5); font-size:11px; margin:0;">Isi data kehadiran siswa</p>
                    </div>
                </div>
            ...
            {{-- ── Body ── --}}
            <form action="{{ route('absensi.store') }}" method="POST" style="padding:24px; display:flex; flex-direction:column; gap:18px;">
                @csrf
                ...
                <div>
                    ...
                    <div style="position:relative;">
                        ...
                        <select name="id_pertemuan"
                                style="width:100%; padding:10px 36px 10px 40px; border:1.5px solid {{ $errors->has('id_pertemuan') ? '#FCA5A5' : '#E5E7EB' }}; border-radius:10px; font-size:14px; color:#111827; background:{{ $errors->has('id_pertemuan') ? '#FEF2F2' : '#F9FAFB' }}; outline:none; box-sizing:border-box; transition:border-color .2s, box-shadow .2s; appearance:none; cursor:pointer;"
                ...
                <div>
                    ...
                    <div style="position:relative;">
                        ...
                        <select name="id_siswa"
                                style="width:100%; padding:10px 36px 10px 40px; border:1.5px solid {{ $errors->has('id_siswa') ? '#FCA5A5' : '#E5E7EB' }}; border-radius:10px; font-size:14px; color:#111827; background:{{ $errors->has('id_siswa') ? '#FEF2F2' : '#F9FAFB' }}; outline:none; box-sizing:border-box; transition:border-color .2s, box-shadow .2s; appearance:none; cursor:pointer;"
                                onfocus="this.style.borderColor='#E8930A'; this.style.boxShadow='0 0 0 3px rgba(232,147,10,0.13)'; this.style.background='#fff';"
                                onblur="this.style.borderColor='#E5E7EB'; this.style.boxShadow='none'; this.style.background='#F9FAFB';">
                    ...
                    <div style="display:grid; grid-template-columns:repeat(4,1fr); gap:8px;">
                        ...
                        <label style="cursor:pointer; position:relative;">
                            <input type="radio" name="status" value="{{ $val }}"
                                   ...
                                   onchange="updateStatusCard(this, '{{ $color }}', '{{ $bg }}', '{{ $border }}', 'create')">
                            <div id="create-status-{{ $val }}"
                                 style="text-align:center; padding:9px 4px; border-radius:10px; border:1.5px solid {{ old('status','hadir') == $val ? $border : '#E5E7EB' }}; background:{{ old('status','hadir') == $val ? $bg : '#F9FAFB' }}; color:{{ old('status','hadir') == $val ? $color : '#9CA3AF' }}; font-size:11.5px; font-weight:700; text-transform:uppercase; letter-spacing:.4px; transition:all .15s; user-select:none;">
                                {{ $lbl }}
                            </div>
                        </label>
                        @endforeach
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3"
                              ...
                              style="width:100%; padding:10px 14px; border:1.5px solid {{ $errors->has('keterangan') ? '#FCA5A5' : '#E5E7EB' }}; border-radius:10px; font-size:14px; color:#111827; background:{{ $errors->has('keterangan') ? '#FEF2F2' : '#F9FAFB' }}; outline:none; resize:none; box-sizing:border-box; font-family:inherit; transition:border-color .2s, box-shadow .2s;"
                              onfocus="this.style.borderColor='#E8930A'; this.style.boxShadow='0 0 0 3px rgba(232,147,10,0.13)'; this.style.background='#fff';"
                              onblur="this.style.borderColor='#E5E7EB'; this.style.boxShadow='none'; this.style.background='#F9FAFB';">{{ old('keterangan') }}</textarea>
                </div>
...
function updateStatusCard(radio, color, bg, border, prefix) {
    ['hadir','izin','sakit','alpha'].forEach(function(v) {
        var card = document.getElementById(prefix + '-status-' + v);
        if (!card) return;
        var c = _statusColors[v];
        if (radio.value === v) {
