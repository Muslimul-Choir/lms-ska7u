    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Mapel</h2>
            ...
            @if ($mapels->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('mapel.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('mapel.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
                            ...
                            <div class="flex flex-col gap-1 flex-1 min-w-[200px]">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="search" value="{{ request('search') }}"
                                        ...
                                        class="w-full pl-9 pr-3 py-2 text-xs bg-white border border-gray-200 rounded-xl text-gray-700 focus:outline-none focus:ring-2 focus:ring-amber-400/30 focus:border-amber-400 transition">
                                </div>
                            </div>
                            <button type="submit"
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Kode Mapel</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Nama Mapel</th>
                            ...
                            @forelse ($mapels as $index => $mapel)
                                <tr class="hover:bg-red-50/30 transition">
                                    <td class="px-6 py-4 text-gray-400 text-xs font-mono">
                                        {{ str_pad($mapels->firstItem() + $index, 3, '0', STR_PAD_LEFT) }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-1 bg-red-50 border border-red-100 rounded-md text-[11px] font-bold text-red-400 tracking-widest line-through decoration-red-300">
                                            {{ $mapel->kode_mapel }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td colspan="6" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">
                                                {{ request()->filled('search') ? 'Tidak ada data yang sesuai filter.' : 'Tempat sampah kosong' }}
                                            </p>
                                            @if(request()->filled('search'))
                                                <a href="{{ route('mapel.trash') }}"
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-100 text-gray-500 text-xs font-semibold rounded-xl border border-gray-200 transition">
