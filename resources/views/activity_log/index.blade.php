    ...
    <div class="bg-gray-50 min-h-screen" x-data="{
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-4">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5 ml-3">Cari aktivitas berdasarkan kata kunci atau jenis
                            aksi</p>
                    </div>
                </div>
                ...
                <form method="GET" action="{{ route('activity-log.index') }}"
                    ...
                    <div class="relative">
                        <select name="filter_aksi"
                            ...
                            <option value="semua" {{ request('filter_aksi') == 'semua' ? 'selected' : '' }}>Semua Aksi
                            </option>
                            <option value="CREATE" {{ request('filter_aksi') == 'CREATE' ? 'selected' : '' }}>CREATE
                            </option>
                            <option value="UPDATE" {{ request('filter_aksi') == 'UPDATE' ? 'selected' : '' }}>UPDATE
                            </option>
                            <option value="DELETE" {{ request('filter_aksi') == 'DELETE' ? 'selected' : '' }}>DELETE
                            </option>
                            <option value="LOGIN" {{ request('filter_aksi') == 'LOGIN' ? 'selected' : '' }}>LOGIN
                            </option>
                            <option value="LOGOUT" {{ request('filter_aksi') == 'LOGOUT' ? 'selected' : '' }}>LOGOUT
                            </option>
                        </select>
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                <th
                                    class="px-6 py-3 text-center text-[11px] font-bold text-gray-500 uppercase tracking-widest w-12">
                                    #</th>
                                <th
                                ...
                                <tr class="hover:bg-amber-50/40 transition group">
                                        ...
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <div>
                                                <p class="font-semibold text-gray-800 text-sm leading-tight">
                                                    {{ $log->user->name ?? 'System/Guest' }}</p>
                                                <p class="text-[10px] text-gray-400">{{ $log->user->email ?? '-' }}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            ...
                                            [$bg, $text, $border] = $badgeMap[strtoupper($log->aksi)] ?? [
                                                'bg-gray-100',
                                                'text-gray-600',
                                                'border-gray-200',
                                            ];
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 {{ $bg }} {{ $text }} border {{ $border }} text-[10px] font-bold uppercase tracking-wide rounded-full">
                                            {{ $log->aksi }}
                                        </span>
                                        <p class="text-[10px] text-gray-500 font-semibold mt-1 tracking-wider">
                                            {{ $log->modul ?? '—' }}</p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <button type="button" @click='openDetail(@json($log))'
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-300 text-xs">Coba gunakan kata kunci atau filter lain.
                                            </p>
                                        </div>
                                    </td>
                                </tr>
                ...
                @if ($logs->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span
                                class="font-semibold text-gray-700">{{ $logs->firstItem() }}–{{ $logs->lastItem() }}</span>
                            dari
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div x-show="showModal" x-transition:enter="transition ease-out duration-200"
                ...
                {{-- Header --}}
                <div
                    class="relative flex items-center justify-between px-6 py-5 overflow-hidden bg-gradient-to-br from-[#6B1A2B] via-[#4A0F1E] to-[#2D0810]">

                    {{-- Decorative Circle --}}
                    <div
                        class="absolute top-[-40px] right-[10px] w-[120px] h-[120px] rounded-full border border-amber-400/20 pointer-events-none">
                    </div>
                    <div
                        class="absolute top-[10px] right-[70px] w-[70px] h-[70px] rounded-full border border-amber-400/10 pointer-events-none">
                    </div>

                    <div class="relative flex items-center gap-3">
                        <div
                            class="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-xl bg-amber-500/20">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                ...
                                <div class="p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
                                    <span
                                        class="block mb-2 text-[10px] font-bold tracking-widest text-gray-400 uppercase">
                                        Aksi
                                    </span>

                                    <template x-if="detailData.aksi === 'CREATE'">
                                        <span
                                            class="inline-flex px-2.5 py-1 text-[10px] font-bold tracking-wide text-blue-700 uppercase border rounded-full bg-blue-50 border-blue-200">
                                            CREATE
                                        </span>
                                    </template>
                                    ...
                                    <template x-if="detailData.aksi === 'UPDATE'">
                                        <span
                                            class="inline-flex px-2.5 py-1 text-[10px] font-bold tracking-wide text-amber-700 uppercase border rounded-full bg-amber-50 border-amber-200">
                                            UPDATE
                                        </span>
                                    </template>
                                    ...
                                    <template x-if="detailData.aksi === 'DELETE'">
                                        <span
                                            class="inline-flex px-2.5 py-1 text-[10px] font-bold tracking-wide text-red-700 uppercase border rounded-full bg-red-50 border-red-200">
                                            DELETE
                                        </span>
                                    </template>
                                    ...
                                    <template x-if="detailData.aksi === 'LOGIN'">
                                        <span
                                            class="inline-flex px-2.5 py-1 text-[10px] font-bold tracking-wide text-emerald-700 uppercase border rounded-full bg-emerald-50 border-emerald-200">
                                            LOGIN
                                        </span>
                                    </template>
                                    ...
                                    <template x-if="detailData.aksi === 'LOGOUT'">
                                        <span
                                            class="inline-flex px-2.5 py-1 text-[10px] font-bold tracking-wide text-gray-700 uppercase border rounded-full bg-gray-100 border-gray-200">
                                            LOGOUT
                                        </span>
                                    </template>
                                </div>
                                ...
                                <div class="p-4 bg-white border border-gray-200 shadow-sm rounded-xl">
                                    ...
                                    <p class="text-sm font-bold text-gray-700" x-text="detailData.modul || '—'">
                                    </p>
                            ...
                            {{-- User --}}
                            <div
                                class="flex items-center gap-3 p-4 bg-white border border-gray-200 shadow-sm rounded-xl">

                                <div
                                    class="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-xl bg-amber-100">
                                ...
                                <div>
                                    <span
                                        class="block mb-1 text-[10px] font-bold tracking-widest text-gray-400 uppercase">
                                        Dilakukan Oleh
                                    </span>
