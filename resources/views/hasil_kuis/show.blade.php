                ...
                <div class="p-6">
                    <div class="flex justify-between items-start mb-6">
                        <div>
                            ...
                            <div class="text-sm text-gray-500 mt-1">{{ $kuis->GuruMapel?->Mapel?->nama_mapel ?? '-' }}
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-5 mb-5">
                        <div class="bg-blue-50 border border-blue-100 p-5 rounded-xl">
                            ...
                            <div class="font-bold text-gray-900 text-base">{{ $hasil->Siswa?->nama_lengkap ?? '-' }}
                            </div>
                        ...
                        <div class="bg-purple-50 border border-purple-100 p-5 rounded-xl">
                            ...
                            <div class="text-xs text-gray-500 mt-1">dari {{ number_format($kuis->nilai_maksimal, 1) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-4 mb-5">
                        <div class="bg-emerald-50 border border-emerald-100 p-4 rounded-xl text-center">
                            ...
                            <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Jawaban Benar
                            </div>
                        </div>
                        <div class="bg-red-50 border border-red-100 p-4 rounded-xl text-center">
                            ...
                            <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Jawaban Salah
                            </div>
                        </div>
                        <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl text-center">
                            ...
                            <div class="text-xs text-gray-600 mt-1 uppercase tracking-wider font-semibold">Total Soal
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 border border-gray-100 p-4 rounded-xl text-sm">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <span class="text-gray-500 text-xs uppercase tracking-wider font-semibold">Waktu
                                    Mulai:</span>
                                <span
                                    class="font-semibold text-gray-700 ml-2">{{ $hasil->waktu_mulai ? \Carbon\Carbon::parse($hasil->waktu_mulai)->format('d M Y, H:i:s') : '-' }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500 text-xs uppercase tracking-wider font-semibold">Waktu
                                    Selesai:</span>
                                <span
                                    class="font-semibold text-gray-700 ml-2">{{ $hasil->waktu_selesai ? \Carbon\Carbon::parse($hasil->waktu_selesai)->format('d M Y, H:i:s') : '-' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6">
                        ...
                        <div class="space-y-5">
                            @foreach ($soalList as $soal)
                                ...
                                <div
                                    class="border-l-4 {{ $isBenar ? 'border-emerald-500 bg-emerald-50' : 'border-red-500 bg-red-50' }} p-5 rounded-r-xl">
                                    <!-- Question Header -->
                                    <div class="flex items-start gap-3 mb-4">
                                        <div
                                            class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold {{ $isBenar ? 'bg-emerald-100 text-emerald-700 border-2 border-emerald-200' : 'bg-red-100 text-red-700 border-2 border-red-200' }}">
                                            {{ $soal->nomor_soal }}
                                        </div>
                                        <div class="flex-1">
                                            ...
                                            <div class="flex items-center gap-2">
                                                @if ($isBenar)
                                                    <span
                                                        class="px-2.5 py-1 text-xs font-bold rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200 inline-flex items-center gap-1">
                                                ...
                                                @else
                                                    <span
                                                        class="px-2.5 py-1 text-xs font-bold rounded-full bg-red-100 text-red-700 border border-red-200 inline-flex items-center gap-1">
                                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd"
                                    ...
                                    <div class="space-y-2.5 ml-13">
                                        @foreach (['A' => $soal->pilihan_a, 'B' => $soal->pilihan_b, 'C' => $soal->pilihan_c, 'D' => $soal->pilihan_d] as $huruf => $pilihan)
                                            ...
                                            <div
                                                class="flex items-start gap-3 p-3 {{ $bgColor }} border-2 {{ $borderColor }} rounded-xl transition-all">
                                                <span
                                                    class="flex-shrink-0 w-6 h-6 rounded-full bg-gray-700 text-white text-xs font-bold flex items-center justify-center">{{ $huruf }}</span>
                                                <span
                                                    class="flex-1 {{ $textColor }} text-sm {{ $isKunci || $isPilihan ? 'font-semibold' : '' }}">{{ $pilihan }}</span>
                                                @if ($icon)
                                    ...
                                    <div class="mt-4 ml-13 p-3 bg-white border border-gray-200 rounded-xl text-sm">
                                        @if ($isBenar)
                                            <span
                                                class="text-emerald-700 font-semibold inline-flex items-center gap-1.5">
                                        ...
                                        @else
                                            <div class="space-y-1.5">
                                                <div class="inline-flex items-center gap-1.5">
                                                    ...
                                                    <span
                                                        class="text-red-600 font-bold">{{ $jawabanSiswa ?? 'Tidak dijawab' }}</span>
                                                </div>
                                                <div class="inline-flex items-center gap-1.5">
                                                    ...
                                                    <span
                                                        class="text-emerald-600 font-bold">{{ $soal->kunci_jawaban }}</span>
                                                </div>
                                            </div>
                                        @endif
                                    </div>
