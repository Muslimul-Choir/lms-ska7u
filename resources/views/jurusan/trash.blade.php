    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400 mt-0.5 tracking-widest uppercase">Data Jurusan Terhapus</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8 bg-slate-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-slate-400 font-medium tracking-wide">
                ...
                <span class="text-slate-300">/</span>
                <a href="{{ route('jurusan.index') }}" class="hover:text-[#1B3A6B] transition">Jurusan</a>
                <span class="text-slate-300">/</span>
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-[#0F2145] to-[#1B3A6B]">
                    <div>
                        <h3 class="text-white font-semibold text-sm tracking-wide">Data Jurusan Terhapus</h3>
                                ...
                                <tr class="hover:bg-slate-50/70 transition group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <span class="font-semibold text-slate-500 text-sm line-through decoration-slate-300">{{ $jurusan->nama_jurusan }}</span>
                                        </div>
                                    </td>
                                    {{-- Keterangan --}}
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 border border-red-100 rounded-lg">
                                            ...
                                            <span class="text-red-500 text-xs font-medium">{{ $jurusan->deleted_at->format('d M Y, H:i') }}</span>
                                        </div>
                                    </td>
                                    {{-- Aksi --}}
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            {{-- Restore --}}
                                            <form action="{{ route('jurusan.restore', $jurusan->id) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-16 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-slate-300 text-xs">Tidak ada data jurusan yang dihapus</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
