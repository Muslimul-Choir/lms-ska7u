        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden ring-1 ring-slate-200">
            ...
            <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-[#0F2145] to-[#1B3A6B]">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <h3 class="text-white font-semibold text-sm tracking-wide">Tambah Pertemuan</h3>
                        <p class="text-blue-200 text-[11px]">Isi data pertemuan baru</p>
                    </div>
                </div>
            ...
            {{-- Body --}}
            <form action="{{ route('pertemuan.store') }}" method="POST" class="px-6 py-5 space-y-4">
                @csrf

                {{-- Jadwal Belajar --}}
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">
                        Jadwal Belajar <span class="text-red-400">*</span>
                    </label>
                    <select name="id_jadwal"
                            class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700
                                   ...
                                   @error('id_jadwal') border-red-400 bg-red-50 @else border-slate-200 @enderror">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">
                        Nomor Pertemuan <span class="text-red-400">*</span>
                    </label>
                    <input type="number" name="nomor_pertemuan" value="{{ old('nomor_pertemuan') }}"
                           ...
                           class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700 placeholder-slate-300
                                  focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]/30 focus:border-[#1B3A6B] transition
                                  @error('nomor_pertemuan') border-red-400 bg-red-50 @else border-slate-200 @enderror">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">
                        Tanggal <span class="text-slate-300 font-normal normal-case tracking-normal">(opsional)</span>
                    </label>
                    <input type="date" name="tanggal" value="{{ old('tanggal') }}"
                           class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700
                                  focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]/30 focus:border-[#1B3A6B] transition
                                  @error('tanggal') border-red-400 bg-red-50 @else border-slate-200 @enderror">
                    @error('tanggal')
                        <p class="mt-1.5 flex items-center gap-1 text-xs text-red-500">
                            <svg class="w-3 h-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                            {{ $message }}
                ...
                <div>
                    ...
                    <select name="status"
                            class="w-full rounded-lg border px-3 py-2.5 text-sm text-slate-700
                                   focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]/30 focus:border-[#1B3A6B] transition
                                   @error('status') border-red-400 bg-red-50 @else border-slate-200 @enderror">
