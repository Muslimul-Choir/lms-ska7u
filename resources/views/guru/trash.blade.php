    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Guru</h2>
            ...
            @if ($gurus->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('guru.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('guru.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                ...
                <span class="text-gray-300">/</span>
                <a href="{{ route('guru.index') }}" class="hover:text-amber-600 transition">Manajemen Guru</a>
                <span class="text-gray-300">/</span>
                    ...
                    <form method="GET" action="{{ route('guru.trash') }}">
                        <div class="flex flex-wrap items-end gap-3">
                            ...
                            <div class="flex flex-col gap-1 flex-1 min-w-[200px]">
                                <label
                                    class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Cari</label>
                                <div class="relative">
                            ...
                            @if (request()->hasAny(['search', 'status_pengajar']))
                            ...
                            @forelse ($gurus as $index => $guru)
                                <tr class="hover:bg-red-50/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold border opacity-60 {{ $badgeClass }}">
                                            {{ $badgeLabel }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div
                                            ...
                                            <div>
                                                <span class="text-red-500 text-[11px] font-medium whitespace-nowrap">
                                                    {{ $guru->deleted_at->format('d M Y, H:i') }}
                                                </span>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('guru.forceDelete', $guru->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmForceDelete(event)"
                                    ...
                                    <td colspan="6" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">
                                                {{ request()->hasAny(['search', 'status_pengajar']) ? 'Tidak ada data yang sesuai filter.' : 'Arsip kosong' }}
                                            </p>
                                            @if (request()->hasAny(['search', 'status_pengajar']))
                                                <a href="{{ route('guru.trash') }}"
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-100 text-gray-500 text-xs font-semibold rounded-xl border border-gray-200 transition">
                                            ...
                                            @else
                                                <p class="text-gray-300 text-xs">Tidak ada guru yang dihapus sementara
                                                </p>
                                            @endif
                                        </div>
                                    </td>
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('guru').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestoreAll('Guru').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmForceDeleteAll('Guru').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
        </script>
