{{-- modal-import guru --}}
<div id="modalImportGuru" class="hidden fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-5">

    {{-- Overlay --}}
    <div onclick="closeImportGuruModal()" class="absolute inset-0 bg-[rgba(45,8,16,0.55)] backdrop-blur-[4px]">
    </div>
        ...
        <div
            ...
            <div class="px-6 py-[18px] flex items-center justify-between relative overflow-hidden"
                ...
                <div class="flex items-center gap-3 relative">
                    ...
                    <div>
                        <h3 class="text-white font-bold text-[15px] m-0 mb-[2px]">Import Data Guru</h3>
                        <p class="text-[rgba(255,255,255,0.5)] text-[11px] m-0">Upload file Excel guru</p>
                    </div>
                </div>

                <button type="button" onclick="closeImportGuruModal()"
            ...
            <form action="{{ route('guru.import') }}" method="POST" enctype="multipart/form-data"
                id="importGuruFormAction" class="p-6 flex flex-col gap-4">
                @csrf
                ...
                <div class="flex items-start gap-2.5 rounded-xl border border-sky-200 bg-sky-50 px-3.5 py-3">
                    <svg class="w-3.5 h-3.5 text-sky-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <code
                            class="block bg-white border border-sky-200 rounded-lg px-3 py-1.5 text-[11px] text-sky-800 font-mono tracking-wide">
                            Nama Lengkap &nbsp;|&nbsp; Email &nbsp;|&nbsp; Status Pengajar
                        </code>
                        <p class="text-[11px] text-sky-600 mt-0.5">
                            Status: <span class="font-semibold">pengajar</span>, <span
                                class="font-semibold">walikelas</span>, atau <span class="font-semibold">keduanya</span>
                        </p>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-[7px]">
                    ...
                    <label
                        ...
                        id="importDropzone">
                        <input type="file" name="file" id="importFileInput" accept=".xlsx,.xls,.csv" class="hidden"
                            onchange="handleImportFileChange(this)">
                        <div id="importPlaceholder" class="flex flex-col items-center gap-2 text-center">
                            <div
                                class="w-10 h-10 rounded-xl bg-gray-100 group-hover:bg-amber-100 flex items-center justify-center transition-all duration-200">
                                <svg class="w-5 h-5 text-gray-400 group-hover:text-amber-500 transition-colors duration-200"
                            ...
                            <div>
                                <p
                                    class="text-xs font-semibold text-gray-500 group-hover:text-amber-600 transition-colors duration-200">
                                    Klik untuk pilih file</p>
                                <p class="text-[10px] text-gray-400 mt-0.5">.xlsx, .xls, atau .csv</p>
                            </div>
                        </div>
                        <div id="importFilePreview" class="hidden flex-col items-center gap-2 text-center">
                            ...
                            <div>
                                <p id="importFileName" class="text-xs font-semibold text-emerald-600"></p>
                ...
                <div class="flex items-center justify-end gap-[10px] pt-[6px] border-t border-gray-100">
                    ...
                    <button type="submit" id="importGuruSubmitBtn"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const importForm = document.getElementById('importGuruFormAction');
        const importBtn = document.getElementById('importGuruSubmitBtn');
