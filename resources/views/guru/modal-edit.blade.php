@php
    $editAction = (old('_modal') === 'edit' && old('edit_id'))
        ? route('guru.update', old('edit_id'))
        : '';
@endphp

<div id="modalEditGuru"
    class="fixed inset-0 z-50 hidden overflow-y-auto"
    aria-labelledby="modalEditGuruTitle"
    role="dialog"
    aria-modal="true">

    {{-- Backdrop --}}
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm"
         onclick="closeEditGuruModal()"></div>
        ...
        <div class="relative w-full max-w-lg rounded-2xl bg-white shadow-xl dark:bg-gray-800">
            ...
            <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    ...
                    <h3 id="modalEditGuruTitle" class="text-base font-semibold text-gray-900 dark:text-white">
                        Edit Guru
                    </h3>
                </div>
            ...
            {{-- Form --}}
            <form id="editGuruForm" action="{{ $editAction }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                <input type="hidden" name="_modal" value="edit">
                <input type="hidden" name="edit_id" id="edit_guru_id" value="{{ old('edit_id') }}">
                ...
                <div class="space-y-4 px-6 py-5">
                    ...
                    {{-- Status Pengajar --}}
                    <div>
                        <label for="edit_status_pengajar" class="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">
                            Status Pengajar <span class="text-red-500">*</span>
                        </label>
                        <select id="edit_status_pengajar" name="status_pengajar"
                            class="w-full rounded-lg border px-3 py-2 text-sm transition focus:outline-none focus:ring-2 focus:ring-amber-500
                            {{ $errors->has('status_pengajar') && old('_modal') === 'edit' ? 'border-red-400 bg-red-50' : 'border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-700 dark:text-white' }}">
