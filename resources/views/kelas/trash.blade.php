                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <select name="id_bagian"
                                    ...
                                    @foreach ($bagianList as $b)
                                        <option value="{{ $b->id }}" {{ request('id_bagian') == $b->id ? 'selected' : '' }}>
                                            {{ $b->nama_bagian }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Jurusan</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Tahun Ajaran</th>
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Wali Kelas</th>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-semibold text-gray-400 text-sm line-through decoration-red-300">
                                                {{ $kelas->Tingkatan?->nama_tingkatan ?? '-' }} {{ $kelas->Bagian?->nama_bagian ?? '-' }}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td colspan="7" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">
                                                {{ request()->hasAny(['id_bagian', 'id_jurusan', 'id_tingkatan', 'id_tahun_ajaran']) ? 'Tidak ada data yang sesuai filter.' : 'Arsip kosong' }}
                                            </p>
                                            @if (request()->hasAny(['id_bagian', 'id_jurusan', 'id_tingkatan', 'id_tahun_ajaran']))
                                                <a href="{{ route('kelas.trash') }}" 
                                                class="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-100 text-gray-500 text-xs font-semibold rounded-xl border border-gray-200 transition">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            @else
                                                <p class="text-gray-300 text-xs">Tidak ada kelas yang dihapus sementara</p>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                ...
                @if ($kelasTrashed->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span class="font-semibold text-gray-700">{{ $kelasTrashed->firstItem() }}–{{ $kelasTrashed->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-gray-700">{{ $kelasTrashed->total() }}</span>
                            entri
                        </p>
                        {{ $kelasTrashed->links() }}
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('kelas').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestoreAll('Kelas').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmForceDeleteAll('Kelas').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
        </script>
