<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-3 bg-slate-50 border-b border-slate-100">
                    <form action="{{ route('kelas.trash') }}" method="GET" class="flex items-center gap-2 max-w-md">
                                ...
                                <tr class="hover:bg-slate-50/70 transition group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <span class="font-semibold text-slate-500 text-sm line-through decoration-slate-300">
                                                {{ $kelas->Tingkatan?->nama_tingkatan ?? '-' }}
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 border border-red-100 rounded-lg">
                                            ...
                                            <span class="text-red-500 text-xs font-medium" title="{{ $kelas->deleted_at }}">
                ...
                @if ($kelasTrashed->hasPages())
                    <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-slate-500">
                            Menampilkan
                            <span class="font-semibold text-slate-700">{{ $kelasTrashed->firstItem() }}–{{ $kelasTrashed->lastItem() }}</span>
                            dari
    ...
    <script>
        function confirmForceDelete(event, nama) {
            if (!confirm(`Hapus permanen kelas "${nama}"?\nTindakan ini TIDAK DAPAT dibatalkan!`)) {
