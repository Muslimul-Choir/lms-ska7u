<x-app-layout>
    ...
    <script>
        ...
        const tableBody = document.getElementById('tingkatanTableBody');
        ...
        function loadData(search = '', page = 1) {
            fetch(`{{ route('tingkatan.index') }}?search=${encodeURIComponent(search)}&page=${page}`, {
                headers: {
            ...
            .then(data => {
                renderTable(data.tingkatans, page);
            })
            .catch(error => console.error('Error:', error));
        }

        // Function to render table
        function renderTable(tingkatans, currentPage) {
                    ...
                    <tr>
                        <td colspan="4" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-slate-400 text-sm font-medium">Belum ada data tingkatan</p>
                                <p class="text-slate-300 text-xs">Klik <span class="font-semibold text-slate-400">Tambah Tingkatan</span> untuk mulai menambahkan data</p>
                            </div>
                        </td>
                    </tr>
                `;
            ...
            tableBody.innerHTML = tingkatans.map((tingkatan, index) => {
                    ...
                    <tr class="hover:bg-slate-50/70 transition group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-md bg-[#1B3A6B]/10 flex items-center justify-center flex-shrink-0">
                                    <span class="text-[#1B3A6B] text-[10px] font-bold uppercase">
                                        ${tingkatan.nama_tingkatan.substring(0, 2)}
                                    </span>
                                </div>
                                <span class="font-semibold text-[#0F2145] text-sm">${tingkatan.nama_tingkatan}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="/tingkatan/${tingkatan.id}" method="POST" onsubmit="return confirm('Yakin ingin menghapus tingkatan ini?')">
