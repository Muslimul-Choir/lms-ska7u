    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Tingkatan</h2>
                    <p class="text-[11px] text-gray-400 mt-0.5 uppercase tracking-widest">Data Terhapus Sementara</p>
                </div>
            </div>

            @if ($tingkatans->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('tingkatan.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('tingkatan.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
        ...
        <div class="max-w-7xl mx-auto space-y-5">
            ...
            <div class="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm">
                ...
                <p class="text-xs tracking-widest font-medium leading-relaxed text-red-700">
                    Data dalam arsip telah dihapus sementara. Gunakan tombol aksi untuk memulihkan data dan menghapus
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-3 bg-gray-50 border-b border-gray-100">
                    <form method="GET" action="{{ route('tingkatan.trash') }}"
                        ...
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th
                                    ...
                                    Dihapus Pada</th>
                                <th
                                    class="px-6 py-3 text-center text-[11px] font-bold text-gray-500 uppercase tracking-widest w-52">
                                    Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($tingkatans as $index => $tingkatan)
                                <tr class="hover:bg-red-50/30 transition">
                                    <td class="px-6 py-4 text-gray-400 text-xs font-mono">
                                        {{ str_pad($tingkatans->firstItem() + $index, 3, '0', STR_PAD_LEFT) }}
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div
                                            ...
                                            <span
                                                class="font-semibold text-gray-400 text-sm line-through decoration-red-300">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            {{-- Restore --}}
                                            <form action="{{ route('tingkatan.restore', $tingkatan) }}"
                                                method="POST" onsubmit="confirmRestore(event)">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit"
                                            ...
                                            {{-- Force Delete --}}
                                            <form action="{{ route('tingkatan.force-delete', $tingkatan) }}"
                                    ...
                                    <td colspan="5" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">
                                                {{ request()->hasAny(['search']) ? 'Tidak ada data yang sesuai filter.' : 'Tempat sampah kosong' }}
                                            </p>
                                            ...
                                            @else
                                                <p class="text-gray-300 text-xs">Tidak ada data tingkatan yang dihapus
                                                    sementara</p>
                                            @endif
                                        </div>
                                    </td>
                ...
                @if ($tingkatans->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span
                                class="font-semibold text-gray-700">{{ $tingkatans->firstItem() }}–{{ $tingkatans->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-gray-700">{{ $tingkatans->total() }}</span>
                            entri
                        </p>
                        {{ $tingkatans->links() }}
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('tingkatan').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestoreAll('Tingkatan').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmForceDeleteAll('Tingkatan').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
        </script>
