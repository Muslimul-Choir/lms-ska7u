    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Guru Mapel</h2>
                    <p class="text-[11px] text-gray-400 mt-0.5 uppercase tracking-widest">Data Terhapus Sementara</p>
                </div>
            </div>

            @if ($guruMapels->total() > 0)
                <div class="flex gap-2 flex-shrink-0">
                    <form action="{{ route('guru_mapel.restoreAll') }}" method="POST" id="restoreAllForm">
                        @csrf
                        @method('PATCH')
                    ...
                    <form action="{{ route('guru_mapel.forceDeleteAll') }}" method="POST" id="forceDeleteAllForm">
                        @csrf
                        @method('DELETE')
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('guru mapel').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                showConfirmRestoreAll('Guru Mapel').then((result) => {
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                showConfirmForceDeleteAll('Guru Mapel').then((result) => {
