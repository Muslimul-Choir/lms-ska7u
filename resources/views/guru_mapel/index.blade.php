<x-app-layout>
    ...
    <script>
        ...
        function renderTable(guruMapels, currentPage) {
            ...
            guruMapels.forEach((guruMapel, index) => {
                const no = (currentPage - 1) * 5 + index + 1;
                html += `
                    <tr class="hover:bg-slate-50/70 transition group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2.5">
                                <div class="w-7 h-7 rounded-md bg-[#1B3A6B]/10 flex items-center justify-center flex-shrink-0">
                                    <span class="text-[#1B3A6B] text-[10px] font-bold uppercase">${guruMapel.mapel.nama_mapel.substring(0, 2)}</span>
                                </div>
                                <span class="font-semibold text-[#0F2145] text-sm">${guruMapel.mapel.nama_mapel}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-slate-600 text-sm">${guruMapel.guru.nama_guru}</td>
                        <td class="px-6 py-4 text-slate-600 text-sm">${guruMapel.kelas.nama_kelas}</td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('guru_mapel.destroy', ':id') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus penugasan guru mapel ini?')">
                                    @csrf
                                    @method('DELETE')
        ...
        function renderPagination(paginationHtml, total) {
            if (paginationContainer) {
                paginationContainer.innerHTML = `
                    <p class="text-xs text-slate-500">
                        Menampilkan <span class="font-semibold text-slate-700">${(currentPage - 1) * 5 + 1}–${Math.min(currentPage * 5, total)}</span> dari <span class="font-semibold text-slate-700">${total}</span> entri
                    </p>
                    ${paginationHtml}`;
            }
        }
        ...
        function attachEditListeners() {
            document.querySelectorAll('.btn-edit').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    const idMapel = this.getAttribute('data-id_mapel');
                    const idGuru = this.getAttribute('data-id_guru');
                    const idKelas = this.getAttribute('data-id_kelas');
                    const idSemester = this.getAttribute('data-id_semester');
                    ...
                    document.getElementById('editIdSemester').value = idSemester;
                    document.getElementById('editForm').action = `{{ url('guru_mapel') }}/${id}`;
        ...
        // Modal create
        document.getElementById('btnTambahGuruMapel').addEventListener('click', () => {
        ...
        document.getElementById('overlayCreate').addEventListener('click', () => {
            modalCreate.style.display = 'none';
        });
