    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Arsip Akun</h2>
                    ...
                    <form method="GET" action="{{ route('users.trash') }}">
                        <div class="flex flex-wrap items-end gap-3">
                            ...
                            <div class="flex flex-col gap-1">
                                <label
                                    class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Role</label>
                                <select name="role"
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($users as $index => $user)
                                <tr class="hover:bg-red-50/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <span
                                            class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold opacity-60
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="{{ route('users.restore', $user->id) }}" method="POST">
                                                ...
                                                @method('PATCH')
                                                <button type="button"
                                                    onclick="confirmRestore(event, '{{ addslashes($user->name) }}')"
                                            ...
                                            <form action="{{ route('users.forceDelete', $user->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="button"
                                                    onclick="confirmForceDelete(event, '{{ addslashes($user->name) }}')"
                                    ...
                                    <td colspan="6" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">
                                                {{ request()->hasAny(['search', 'role']) ? 'Tidak ada data yang sesuai filter.' : 'Arsip kosong' }}
                                            </p>
                                            @if (request()->hasAny(['search', 'role']))
                                                <a href="{{ route('users.trash') }}"
                                                    class="inline-flex items-center gap-1.5 px-3 py-2 bg-white hover:bg-gray-100 text-gray-500 text-xs font-semibold rounded-xl border border-gray-200 transition">
                                            ...
                                            @else
                                                <p class="text-gray-300 text-xs">Tidak ada akun yang dihapus sementara
                                                </p>
                                            @endif
                                        </div>
                                    </td>
    ...
    @push('scripts')
        <script>
            ...
            function confirmRestore(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestore('user').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmRestoreAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmRestoreAll('User').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
            ...
            function confirmForceDeleteAll(event) {
                event.preventDefault();
                const form = event.target.closest('form');
                showConfirmForceDeleteAll('User').then((result) => {
                    if (result.isConfirmed) form.submit();
                });
            }
        </script>
