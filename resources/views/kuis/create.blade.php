                ...
                <form action="{{ route('kuis.store') }}" method="POST">
                        ...
                        <div>
                            ...
                            <select name="id_guru_mapel" required
                                ...
                                @foreach($guruMapelList as $gm)
                                    <option value="{{ $gm->id }}" {{ old('id_guru_mapel') == $gm->id ? 'selected' : '' }}>
                                        {{ $gm->Mapel?->nama_mapel ?? '-' }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="nilai_maksimal" value="{{ old('nilai_maksimal', 100) }}" required min="1" max="100" step="0.1"
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                placeholder="100">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="datetime-local" name="batas_mulai" value="{{ old('batas_mulai') }}" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Batas Selesai <span class="text-red-500">*</span></label>
                            <input type="datetime-local" name="batas_selesai" value="{{ old('batas_selesai') }}" required
                                class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                    ...
                    <div class="p-4 mb-4 text-sm text-blue-700 bg-blue-100 rounded-lg flex items-start gap-2">
                        ...
                        <div>
                            <strong>Catatan:</strong> Kuis akan dibuat dengan status <strong>Draft</strong>. Anda dapat menambahkan soal terlebih dahulu sebelum mempublikasikan.
                        </div>
                    </div>
