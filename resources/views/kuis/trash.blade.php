    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <a href="{{ route('kuis.index') }}" class="mb-4 inline-flex items-center gap-2 bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    ...
                                    @if($kuis->deskripsi)
                                        <div class="text-sm text-gray-500">{{ Str::limit($kuis->deskripsi, 50) }}</div>
                                    @endif
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-2">
                                        <form action="{{ route('kuis.restore', $kuis->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="inline-flex items-center gap-1 text-green-600 hover:text-green-900" onclick="return confirm('Pulihkan kuis ini?')" title="Pulihkan">
                                        ...
                                        <form action="{{ route('kuis.force-delete', $kuis->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="inline-flex items-center gap-1 text-red-600 hover:text-red-900" onclick="return confirm('Hapus permanen kuis ini? Tindakan ini tidak dapat dibatalkan!')" title="Hapus Permanen">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
