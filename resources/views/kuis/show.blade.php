            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="grid grid-cols-4 gap-4 mb-6">
                    <div class="bg-blue-50 p-4 rounded-lg">
                        ...
                        <div class="font-bold text-gray-900">{{ $kuis->GuruMapel?->Mapel?->nama_mapel ?? '-' }}</div>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <div class="font-semibold">{{ \Carbon\Carbon::parse($kuis->batas_mulai)->format('d M Y, H:i') }}</div>
                    </div>
                    <div>
                        <div class="text-sm text-gray-600 mb-1">Batas Selesai</div>
                        <div class="font-semibold">{{ \Carbon\Carbon::parse($kuis->batas_selesai)->format('d M Y, H:i') }}</div>
                    </div>
                </div>

                <div class="flex gap-2">
                    <a href="{{ route('kuis.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-2.5 px-4 rounded-lg inline-flex items-center gap-2 transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                        </svg>
                        Kembali
                    </a>
                    <a href="{{ route('soal_kuis.index', $kuis) }}" class="bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2.5 px-4 rounded-lg inline-flex items-center gap-2 shadow-sm transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                        </svg>
                        Kelola Soal
                    </a>
                    <a href="{{ route('kuis.edit', $kuis) }}" class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2.5 px-4 rounded-lg inline-flex items-center gap-2 shadow-sm transition-all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="font-medium text-gray-900">{{ $hasil->Siswa?->nama_lengkap ?? '-' }}</div>
                                        <div class="text-sm text-gray-500">{{ $hasil->Siswa?->nis ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-lg font-bold text-purple-700">{{ number_format($hasil->nilai, 1) }}</span>
                                        <span class="text-sm text-gray-500">/ {{ number_format($kuis->nilai_maksimal, 1) }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        <span class="text-green-600 font-semibold inline-flex items-center gap-1">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('hasil_kuis.show', [$kuis, $hasil]) }}" class="text-blue-600 hover:text-blue-900 inline-flex items-center gap-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
