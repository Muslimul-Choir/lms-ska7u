<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="font-bold text-[15px] text-gray-800 tracking-wide leading-none">Master Tahun Ajaran</h2>
    ...
    <div class="py-7 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <nav class="flex items-center gap-1.5 text-xs text-gray-400 font-medium">
                ...
                <span class="text-gray-300">/</span>
                <span class="text-gray-600 font-semibold">Tahun Ajaran</span>
            </nav>
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h3 class="font-semibold text-gray-800 text-sm tracking-wide">Daftar Tahun Ajaran</h3>
                        <p class="text-gray-400 text-xs mt-0.5">Kelola data tahun akademik</p>
                    </div>
                    ...
                    <table class="min-w-full text-sm">
                        <thead>
                            <tr class="bg-gray-50 border-b border-gray-200">
                                ...
                                <th class="px-6 py-3 text-left text-[11px] font-bold text-gray-500 uppercase tracking-widest">Nama Tahun</th>
                                ...
                                <tr class="hover:bg-amber-50/40 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-semibold text-gray-800 text-sm">{{ $tahun->nama_tahun }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-1.5">
                                            ...
                                            <button type="button"
                                                ...
                                                data-id="{{ $tahun->id }}"
                                                data-route="{{ route('tahunajaran.update', $tahun->id) }}"
                                                data-nama="{{ $tahun->nama_tahun }}"
                                                data-is-aktif="{{ $tahun->is_aktif ? '1' : '0' }}"
                                                title="Edit"
                                            ...
                                            {{-- Delete --}}
                                            <form action="{{ route('tahunajaran.destroy', $tahun->id) }}" method="POST"
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-20 text-center">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-gray-400 text-sm font-semibold">Belum ada data tahun ajaran</p>
                                            <p class="text-gray-300 text-xs">Klik <span class="font-semibold text-gray-400">+ Tambah Tahun</span> untuk mulai menambahkan</p>
                                            <button type="button" onclick="openCreateModal()"
                ...
                @if ($tahunAjarans->hasPages())
                    <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-4">
                        <p class="text-xs text-gray-500">
                            Menampilkan
                            <span class="font-semibold text-gray-700">{{ $tahunAjarans->firstItem() }}–{{ $tahunAjarans->lastItem() }}</span>
                            dari
                            <span class="font-semibold text-gray-700">{{ $tahunAjarans->total() }}</span>
                            entri
                        </p>
                        {{ $tahunAjarans->links() }}
                    </div>
    ...
    @push('scripts')
        <script>
            ...
            btnSearch.addEventListener('click', () => {
                window.location.href = `{{ route('tahunajaran.index') }}?search=${encodeURIComponent(searchInput.value)}`;
            });
            ...
            function openEditModal(button) {
                ...
                document.getElementById('edit_nama_tahun').value = d.nama;
