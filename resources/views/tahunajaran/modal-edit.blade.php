        ...
        <div style="background:#fff; border-radius:18px; box-shadow:0 24px 60px rgba(107,26,43,0.22), 0 4px 16px rgba(0,0,0,0.08); overflow:hidden; border:1px solid rgba(107,26,43,0.1);">
            ...
            <div style="padding:18px 24px; background:linear-gradient(135deg,#6B1A2B 0%,#4A0F1E 55%,#2D0810 100%); display:flex; align-items:center; justify-content:space-between; position:relative; overflow:hidden;">
                ...
                <div style="display:flex; align-items:center; gap:12px; position:relative;">
                    ...
                    <div>
                        <h3 style="color:#fff; font-weight:700; font-size:15px; margin:0 0 2px;">Edit Tahun Ajaran</h3>
                        <p style="color:rgba(255,255,255,0.5); font-size:11px; margin:0;">Perbarui data tahun akademik</p>
                    </div>
                </div>
            ...
            <form id="formEdit" action="" method="POST" style="padding:24px; display:flex; flex-direction:column; gap:18px;">
                ...
                {{-- Status Aktif --}}
                <div>
                    <label id="labelIsAktif"
                           style="display:flex; align-items:center; gap:10px; cursor:pointer; padding:12px 14px; background:#F9FAFB; border:1.5px solid #E5E7EB; border-radius:10px; transition:border-color .2s;"
...
<script>
function openEditModal(id, namaTahun, isAktif) {
    var m = document.getElementById('modalEdit');
    document.getElementById('formEdit').action = '/tahunajaran/' + id;
    document.getElementById('editNamaTahun').value  = namaTahun || '';
