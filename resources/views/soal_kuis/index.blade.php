<x-app-layout>
    ...
    <div class="bg-gray-50 min-h-screen" x-data="{ showCreateModal: false, showEditModal: false, editData: {} }">
        <div class="max-w-7xl mx-auto space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6">
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <div class="text-sm text-gray-600 mt-1">
                                    {{ $kuis->GuruMapel?->Mapel?->nama_mapel ?? '-' }} • {{ $kuis->durasi }} menit •
                                    Nilai Maks: {{ number_format($kuis->nilai_maksimal, 1) }}
                                </div>
                            </div>
                            <div>
                    ...
                    @if ($sudahDikerjakan)
                        <div
                            class="flex items-start gap-2 p-4 mb-4 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-xl">
                            ...
                            <div>
                                <strong>Peringatan:</strong> Kuis ini sudah dikerjakan oleh siswa. Soal tidak dapat
                                diubah atau dihapus.
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="space-y-4">
                        @forelse($soalList as $soal)
                            <div
                                class="border border-gray-100 rounded-xl p-5 hover:shadow-md hover:border-amber-200 transition-all bg-white">
                                <div class="flex items-start gap-4">
                                    <div
                                        class="flex-shrink-0 w-10 h-10 bg-amber-100 rounded-full flex items-center justify-center font-bold text-amber-700">
                                        {{ $soal->nomor_soal }}
                                    </div>
                                    <div class="flex-1">
                                        <div class="font-semibold text-gray-900 mb-3 text-sm">{{ $soal->pertanyaan }}
                                        </div>
                                        ...
                                        <div
                                            class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xs font-bold">
                                    ...
                                    @if (!$sudahDikerjakan)
                                        <div class="flex-shrink-0 flex items-center gap-2">
                                            <button @click="editData = {{ json_encode($soal) }}; showEditModal = true"
